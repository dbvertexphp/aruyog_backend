    ...
    socket.on("new message", (newMessageRecieved) => {
      try {
        ...
        chat.users.forEach((user) => {
          if (user._id == newMessageRecieved.response.sender._id) return;

          socket.in(room_id).emit("message received", newMessageRecieved);

          const OfflineId = generateOfflineId(8);
          socket.broadcast.emit("offline_room", {
            OfflineId,
            userId: user._id,
            myId: newMessageRecieved.response.sender._id,
          });
        });
      } catch (error) {
        ...
        for (const user of response.chat.users) {
          if (user._id !== response.sender._id) {
            ...
            if (userInfo.length > 0) {
              const chatStatus = userInfo[0].Chat_Status;
              console.log(chatStatus);
              console.log(userInfo);

              // If user is online, update the message read status
              if (chatStatus === "Online") {
                io.emit("message read update", user._id);
