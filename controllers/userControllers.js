...
const registerUser = asyncHandler(async (req, res, next) => {
      ...
      upload.single("profile_pic")(req, res, async (err) => {
            ...
            const { token, id } = await createConnectyCubeUser(
                  mobile,
                  password,
                  email,
                  full_name,
            ...
            if (user) {
                  ...
                  try {
                        ...
                        console.error(
                              "Failed to update admin dashboard:",
                              error
                        );
                  }
...
const bank_Detail_create = asyncHandler(async (req, res) => {
      const { bankName, accountNumber, ifscCode, bankAddress, teacherName } =
            req.body;
      const userId = req.headers.userID; // Assuming you have user authentication middleware
      ...
      try {
            ...
            const transformedCoursesPromises = courses.map(async (course) => {
                  ...
                  if (transformedCourse.startTime) {
                        transformedCourse.startTime = moment(
                              transformedCourse.startTime
                        ).format("DD/MM/YYYY");
                  }
                  if (transformedCourse.endTime) {
                        transformedCourse.endTime = moment(
                              transformedCourse.endTime
                        ).format("DD/MM/YYYY");
                  }

                  // Fetch subcategory name
                  const category = await Category.findById(
                        transformedCourse.category_id
                  );
                  const subCategory = category.subcategories.id(
                        transformedCourse.sub_category_id
                  );

                  transformedCourse.category_name = category.category_name;
                  transformedCourse.subcategory_name =
...
const addMasterPayment = asyncHandler(async (req, res, next) => {
      ...
      if (isNaN(master) || master === 0) {
            return next(
                  new ErrorHandler(
                        "Please enter a valid master payment amount.",
                        400
                  )
            );
      }
...
const updateUserPayment = async (req, res, next) => {
      ...
      if (!userId || !payment_id) {
            return next(
                  new ErrorHandler("Please provide userId and payment_id.", 400)
            );
      }
      ...
      try {
            ...
            const coursesWithAvailability = courses.map((course) => {
                  ...
                  if (course.type === "group_course") {
                        courseAvailable =
                              course.userIds.length < 3 ? "available" : "full";
                  } else if (course.type === "single_course") {
                        courseAvailable =
                              course.userIds.length < 1 ? "available" : "full";
                  }
                  return {
                        ...course.toObject(),
                        courseAvailable,
      ...
      try {
            ...
            if (favorite) {
                  ...
                  if (Array.isArray(favorite.teacher_ids)) {
                        ...
                        favorite.teacher_ids = favorite.teacher_ids.filter(
                              (id) => !teacherIdsArray.includes(id.toString())
                        );
...
const getTeachersBySubcategory = asyncHandler(async (req, res) => {
      ...
      try {
            ...
            const teacherIds = [
                  ...new Set(
                        courses.map((course) =>
                              course.teacher_id._id.toString()
                        )
                  ),
            ];
            ...
            if (search) {
                  ...
                  teachers = teachers.filter((teacher) =>
                        searchRegex.test(teacher.full_name)
                  );
            }
            ...
            const favoriteTeacherIds = favorite
                  ? favorite.teacher_ids.map((id) => id.toString())
                  : [];
            ...
            const teachersWithDetails = await Promise.all(
                  teachers.map(async (teacher) => {
                        ...
                        return {
                              ...teacher.toObject(),
                              favorite: favoriteTeacherIds.includes(
...
const getCoursesByUserId = asyncHandler(async (req, res) => {
      ...
      try {
            ...
            const courses = transactions.map(
                  (transaction) => transaction.course_id
            );
...
const getStudentsPayment = asyncHandler(async (req, res) => {
      ...
      try {
            ...
            console.error(
                  "Error fetching students for teacher:",
                  error.message
            );
            res.status(500).json({ message: "Internal Server Error" });
      }
...
const getTotalAmount = asyncHandler(async (req, res) => {
      ...
      try {
            ...
            const currentMonthAmount = groupedTransactions[currentMonthKey]
                  ? groupedTransactions[currentMonthKey].reduce(
                          (sum, txn) => sum + txn.amount,
                          0
                    )
                  : 0;

            const monthlyAmounts = Object.keys(groupedTransactions).map(
                  (monthKey) => {
                        const monthName = moment(monthKey, "YYYY-MM").format(
                              "MMM"
                        );
                        return {
                              month: `${monthName} ${moment(
                                    monthKey,
                                    "YYYY-MM"
                              ).format("YYYY")}`,
                              amount: groupedTransactions[monthKey].reduce(
                                    (sum, txn) => sum + txn.amount,
                                    0
                              ),
                        };
            ...
            console.error(
                  "Error fetching total amounts for teacher:",
                  error.message
            );
            res.status(500).json({ message: "Internal Server Error" });
      }
